...
for epoch in np.arange(0, nEpoch):
        ...
        error = (hypo - y_train)
        gradient = (x_train.T.dot(error))-(lamda*w)
        if (lamda!=0):
...
'''
tprb = conf_matrix[0][0]/(conf_matrix[0][0] + conf_matrix[1][0])
print(tprb)
fprb = conf_matrix[0][1]/ (conf_matrix[0][1] + conf_matrix[1][1])
print(fprb)
fpr = [0, fprb, 1 ]
tpr = [0, tprb, 1 ]
...
plt.plot(fpr, tpr, 'green', label = 'Accuracy = %0.2f' %roc_auc)
